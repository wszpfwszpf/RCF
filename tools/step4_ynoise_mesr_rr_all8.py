# tools/step4_ynoise_mesr_rr_all8.py
# 日期：2026-01-08
# 中文说明：
# 该脚本用于在 E-MLB 的 8 个子集（day/night × ND00/04/16/64）上统计 ESR 指标（用于 YNoise baseline），
# 口径与 step4_baf_mesr_rr_all8.py 完全一致：
# - 先按固定事件数 N_PER_PACKET 将每个文件切成多个 packet
# - 每个 packet 计算一次 ESR(v1)
# - 文件级 mesr = mean(packet_esr)
# - 子集级 mesr = sum(file_mesr) / 文件数（按文件平均，不做 packet 加权）
# 同时根据 keepmask 计算 rr（保留率）：文件 rr = mean(packet_rr)，子集 rr = mean(file_rr)
#
# keepmask：每个文件一个 npz，key="keepmask"，单列 0/1（与原始事件顺序对齐）
#
# 使用方式：
# 1) 先跑 YNoise 的 step4（生成 keepmask）到 KEEP_DIR
# 2) 再运行本脚本统计 all8 的 mesr/rr
#
# 本脚本参照：tools/step4_baf_mesr_rr_all8.py 的格式与实现习惯，仅替换为 YNoise 的 keepmask 目录与输出路径。
...
KEEP_DIR = Path(r"../data/emlb_ynoise_verify/keepmask")       # YNoise keepmask npz 目录（改成你 YNoise step4 输出的位置）
OUT_DIR = Path(r"../baselines/step4_ynoise_mesr_rr_all8")
OUT_CSV = OUT_DIR / "ynoise_mesr_rr_all8_filemean.csv"
...
def _find_keepmask_npz(aedat4_path: Path, keep_dir: Path, split: str, nd: str) -> Optional[Path]:
    """
    与 baseline pipeline 命名保持一致，优先：
      <stem>_<split>_<nd.lower()>_keepmask.npz
    其次：
      <stem>_<nd.lower()>_keepmask.npz
    最后做轻量 fallback glob。
    """
...
def _load_keepmask_1d(npz_path: Path) -> np.ndarray:
    ...
    # object list -> concat（有些 step4 会按 bin 存成 object list）
...
def _eventstore_to_xy(events) -> tuple[np.ndarray, np.ndarray]:
    # 与 step4_baf 版本保持一致：优先 numpy 视图
