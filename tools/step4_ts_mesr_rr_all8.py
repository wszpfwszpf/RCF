# tools/step4_ts_mesr_rr_all8.py
# 日期：2026-01-08
# 中文说明：
# 该脚本用于在 E-MLB 的 8 个子集（day/night × ND00/04/16/ND64）上统计 ESR 指标（用于 TS baseline），
...
#
# 使用方式：
# 1) 先跑 TS 的 step4（生成 keepmask）到 KEEP_DIR
# 2) 再运行本脚本统计 all8 的 mesr/rr
#
# 本脚本参照：tools/step4_baf_mesr_rr_all8.py 的格式与实现习惯，仅替换为 TS 的 keepmask 目录与输出路径。
...
KEEP_DIR = Path(r"../data/emlb_ts_verify/keepmask")       # TS keepmask npz 目录（改成你 TS step4 输出的位置）
OUT_DIR = Path(r"../baselines/step4_ts_mesr_rr_all8")
OUT_CSV = OUT_DIR / "ts_mesr_rr_all8_filemean.csv"
