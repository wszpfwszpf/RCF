...
# -----------------------------
# 方法1：直接设置字体族（优先使用 Times New Roman）
plt.rcParams['font.family'] = 'Times New Roman'

# 设置数学公式字体（LaTeX风格）
plt.rcParams['mathtext.fontset'] = 'stix'  # 使用STIX字体，类似Times New Roman的数学字体
plt.rcParams['mathtext.default'] = 'regular'

# 设置其他字体相关参数
plt.rcParams['font.size'] = 10  # 基础字体大小
plt.rcParams['axes.titlesize'] = 11  # 轴标题字体大小
...
def plot_main_figure(eta, retention, mesr_rcf, mesr_rnd, delta, title, out_path):
    ...
    # 修改1：进一步减小图片宽度，使图形更窄
    # 原figsize=(5.2, 4.4)，现在进一步减小宽度
    fig = plt.figure(figsize=(4.8, 4.4))  # 宽度从5.2减小到4.8
    gs = fig.add_gridspec(2, 1, height_ratios=[2.2, 1.2], hspace=0.08)  # hspace进一步减小，更紧凑
    ...
    # 上图：曲线对比
    # -----------------------------
    ax1.plot(eta, mesr_rcf, marker="o", linewidth=1.6, label="RCF", markersize=4)  # 减小标记大小
    ax1.plot(eta, mesr_rnd, marker="o", linewidth=1.6, label="Random deletion", markersize=4)
    ax1.set_ylabel("mESR", fontname='Times New Roman', fontsize=10)

    # 修改图例位置和大小，避免遮挡数据
    ax1.legend(loc="best", frameon=True, fontsize=8, prop={'family': 'Times New Roman'})
    ...
    # 减小左边界，让图形内容更靠左
    ax1.set_position([0.12, ax1.get_position().y0, 0.88, ax1.get_position().height])
    ...
    if len(eta) >= 2:
        ...
        bar_w = 0.5 * min_step  # 从0.55减小到0.5，柱子更窄
    else:
        bar_w = 0.025  # 从0.03减小到0.025

    ax2.bar(eta, delta, width=bar_w)
    ax2.set_xlabel(r"Threshold $\eta$", fontname='Times New Roman', fontsize=10)
    ax2.set_ylabel(r"$\Delta$mESR", fontname='Times New Roman', fontsize=10)
    ...
    # 减小左边界，与上图对齐
    ax2.set_position([0.12, ax2.get_position().y0, 0.88, ax2.get_position().height])

    # 关键：收紧 x 轴左右留白，让柱子看起来不"松"
    # 让左右边界只比最左/最右多一点点
    if len(eta) >= 2:
        pad = 0.7 * bar_w  # 从0.8减小到0.7，进一步收紧
    else:
        pad = 0.015  # 从0.02减小到0.015
    ...
    upper = y_max * 1.25 + 1e-9  # 从1.28减小到1.25，减小上方空白
    ax2.set_ylim(lower, upper)

    y_range = upper - lower
    offset = 0.035 * y_range  # 从0.04减小到0.035
    ...
    fig.suptitle(title, y=0.98, fontsize=10, fontname='Times New Roman')  # y从0.965调整到0.98，fontsize从11减小到10

    # tight_layout 预留标题空间；不要 bbox_inches="tight" 过度裁切
    fig.tight_layout(rect=[0, 0, 1, 0.96])  # rect从[0, 0, 1, 0.93]调整到[0, 0, 1, 0.96]
    fig.savefig(out_path, dpi=300, bbox_inches='tight', pad_inches=0.05)  # 添加pad_inches参数控制边界
    plt.close(fig)


def stitch_images_grid(image_paths, out_path, n_rows=2, n_cols=4, pad=6, bg=(255, 255, 255)):
    """
    把若干张图拼成固定行列的大图（论文版留白更舒服）。

    修改：减小水平间隔(pad)，使子图间更紧凑
    """
    ...
    # 修改：减小pad值，使水平间隔更小
    # 原pad=12，现在减小到6（垂直间隔保持不变）
    canvas_w = n_cols * target_w + (n_cols - 1) * pad
    canvas_h = n_rows * target_h + (n_rows - 1) * 12  # 垂直间隔保持12不变
...
def main():
    ...
    # 修改：传递更小的pad值
    stitch_images_grid(out_imgs, big_out, n_rows=N_ROWS, n_cols=N_COLS, pad=6)
